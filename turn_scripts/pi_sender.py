...
file_location = '~/Documents/robot/turn_scripts/'
...
def send_command(command, ssh):
        print(command)
        print('echo "' + command + '" >> ' + file_location + '/' + file_name)
...
def generate_time_stamp():
        ...
        time_stamp = int(math.floor((current_time * 10000) % 10000000000))
        if (len(str(time_stamp)) == 9):
                return time_stamp * 10
